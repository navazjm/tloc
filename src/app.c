 ...
 */
void
tloc_app_parse_cmd_args(TLOC_App *app, int argc, char **argv)
{
...
void
tloc_app_count_lines_of_code(TLOC_App *app)
{
...
void
tloc_app_count_lines_of_code_file(TLOC_App *app, const char *file_path)
{
    FILE *fp;
    char *line = NULL;
    size_t len = 0;
    ...
    while ((read = getline(&line, &len, fp)) != -1)
    {
        ...
        if (strncmp(found_supported_language->multiline_comment_start, line_trimmed,
                    strlen(found_supported_language->multiline_comment_start))
            == 0)
        {
        ...
        if (found_supported_language->singleline_comment
            && strncmp(found_supported_language->singleline_comment, line_trimmed,
                       strlen(found_supported_language->singleline_comment))
                   == 0)
        {
            file_summary.comment_lines++;
            continue;
...
void
tloc_app_count_lines_of_code_dir(TLOC_App *app)
{
    if (app->opts.use_git)
    {
 ...
 */
void
tloc_app_count_lines_of_code_dir_git(TLOC_App *app)
{
    char *git_cmd;
...
void
tloc_app_count_lines_of_code_dir_recursion(TLOC_App *app, const char *dir_path)
{
    struct dirent *entry;
...
void
tloc_app_display_results(TLOC_App *app)
{
...
void
tloc_app_display_results_by_file(TLOC_App *app)
{
...
void
tloc_app_display_results_by_language(TLOC_App *app)
{
