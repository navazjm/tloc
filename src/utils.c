 ...
 */
void
tloc_utils_split_cmd_arg(const char *input, char *tokens[2])
{
 ...
 */
char *
tloc_utils_string_trim_whitespace(char *str)
{
    char *end;
 ...
 */
bool
tloc_utils_string_ends_with(const char *str, const char *suffix)
{
 ...
 */
void
tloc_utils_string_strip_prefix(char *dest, const char *prefix)
{
 ...
 */
void
tloc_utils_format_file_path_by_pp_option(char *file_path, TLOC_PP_Option pp_opt)
{
    switch (pp_opt)
    {
    case TLOC_PP_NONE:
    { // trim file path to just filename.ext
    ...
    case TLOC_PP:
    { // trim file path to parent/filename.ext
        char *last_slash = strrchr(file_path, '/');
        if (last_slash)
        {
    ...
    case TLOC_PP_S:
    { // trim file path to p/filename.ext, p being the first letter of parent dir
        char *last_slash = strrchr(file_path, '/');
        if (last_slash)
        {
            *last_slash = '\0'; // Temporarily terminate the string before the last slash
    ...
    case TLOC_PP_AS:
    { // trim file path to only show the first letter of all directories
        char *p = file_path;
        char *next_slash;
 ...
 */
void
tloc_utils_normalize_file_path(char *file_path)
{
